...
var itemTable = {
  props: ['text', 'anotherText'],
  template: '<table><tr>{{text}}-{{anotherText}}</tr></table>'
}

var itemList = {
  props: ['text', 'anotherText'],
  template: '<li>{{text}}-{{anotherText}}</li>'
}
...
new Vue({
  ...
  methods: {
    ...
    changeComp: function(){
      this.viewComp = this.tableComp = this.tableComp === 'child-list' ? 'child-table' : 'child-list';
    }
  }
})
